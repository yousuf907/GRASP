...
class MNModel(object):
    ...
    ### -------- DERpp Rehearsal --------- ###
    ### ---------------------------------- ###
    def derpp_rehearsal(self, replay_loader, new_class_list, num_iters):
        start_time = time.time()
        ...
        for batch_ix, batch in enumerate(replay_loader):
            batch_x, labels_batch = batch[0], batch[1]
            ...
            if num_old < len(labels_batch):
                data_new = batch_x[~idxs_old0]
                data_old = batch_x[idxs_old0]
                ...
                output = classifier_F(batch_x)  # 64 x 80 x 14 x 14
