...
import pickle
import copy
import utils_pq as utils
from retrieve_any_layer import ModelWrapper
from scipy.stats import mode
from collections import defaultdict
from utils_pq import RunningMean
...
class MNModel(object):
    ...
    ### computing distance to class means for grasp rehearsal ###
    ...
    ## ---------------------------- ##
    ## -GRASP-Easy Rhearsal Policy- ##
    ## ---------------------------- ##
    def grasp_easy_sampling(self, dist_dict, class_list, num_iters):
        rehearsal_idxs = []
        count = 0
        budget = self.sleep_bs*num_iters # 512*iters
        while count < budget:
    ...
    ## ---------------------------- ##
    ## -GRASP-Hard Rhearsal Policy- ##
    ## ---------------------------- ##
    def grasp_hard_sampling(self, dist_dict, class_list, num_iters):
        rehearsal_idxs = []
        count = 0
        budget = self.sleep_bs*num_iters # 512*iters
        while count < budget:
    ...
    def kmeans_dist(self, opq, pq, latent_dict, rehearsal_ixs):
        start_time = time.time()
        dist_dict = {}
        model_clone = copy.deepcopy(self.classifier_F)
        model_clone.cuda()
        ...
        for c in class_list:
            class_codes = codes[lbl == c]  # filter codes by class c # NC x 14 x 14 x 80
            class_ixs = ixs[lbl == c]
            class_codes = np.reshape(class_codes, (
                class_codes.shape[0] * self.num_feats * self.num_feats, self.num_codebooks)) # 196NC x 8            
            recon = pq.decode(class_codes) # PQ # 196NC x 80
            recon = opq.reverse_transform(recon) # 196NC x 80
            recon = np.reshape(recon, (-1, self.num_feats, self.num_feats, self.num_channels))  # NCx14x14x80
            recon = torch.from_numpy(np.transpose(recon, (0, 3, 1, 2))).cuda()  # NCx80x14x14
            features = model_clone.get_penultimate_feature(recon) # NCx1280
            features = F.normalize(features, p=2.0, dim=1) # NC x 1280
            features = features.detach().cpu().numpy()
            kmeans = KMeans(n_clusters=3).fit(features)
            centers = kmeans.cluster_centers_ # n_clusters x 1280
            dist_pair = euclidean_distances(centers, features) # n_clusters x NC
            min_dist = dist_pair.min(0) # NC
            dist_dict[c] = [min_dist.squeeze(), class_ixs]
            ts = int((time.time() - start_time)/60) # in mins
        print("Elasped time in pairwise distance computation is:", ts)
        return dist_dict, class_list


    ### KMeans Sampling -- Class-balanced ##
    def sample_kmeans(self, dist_dict, class_list, num_iters):
        rehersal_idxs = []
        count = 0
        budget = self.sleep_bs*num_iters # 512*iters
        while count < budget:
            for i in range(len(class_list)):
                ...
                sel_idx = np.random.choice(ixs_current_class, size=1, replace=False, p=p_curr_class)
                count += 1
                rehersal_idxs.append(torch.from_numpy(sel_idx))
                if count >= budget:
                    break
        rehersal_idxs = torch.cat(rehersal_idxs, dim=0)
        rehersal_idxs = torch.tensor(rehersal_idxs[:budget], dtype=torch.int32)
        assert len(rehersal_idxs) <= budget
        rehersal_idxs= np.array(rehersal_idxs, dtype=np.int32)
        return rehersal_idxs


    ### ----------------------------------- ###
    ### -------- KMeans Rehearsal --------- ###
    ### ----------------------------------- ###
    def rehearsal_km(self, opq, pq, latent_dict, rehearsal_ixs, num_iters):
        start_time = time.time()
        ...
        ### KMeans Method ### 
        dist_dict, class_list = self.kmeans_dist(opq, pq, latent_dict, rehearsal_ixs)
        to_be_replayed = self.sample_kmeans(dist_dict, class_list, num_iter)
    ...
    ### ---------------------------------------- ###
    def rehearsal_mir(self, opq, pq, latent_dict, rehearsal_ixs, 
    ...
    ### Min Margin Sampling ###
    ### ------------------- ###
    def sample_min_logit(self, opq, pq, latent_dict, rehearsal_ixs, batch_size, 
        ...
        if step == 0:
            ...
            with torch.no_grad():
                ...
                vals = abs(conf) # min values are prioritized
    ...
    ### ---------------------- ###
    ### Min Confidence Sampling ###
    ### ----------------------- ###
    def sample_min_conf(self, opq, pq, latent_dict, rehearsal_ixs, batch_size, 
        labels=None, margins=None, step=0):
        keep = batch_size
        start_time = time.time()
        num_data = len(rehearsal_ixs)
        if step == 0:
            ...
            with torch.no_grad():
                ...
                p = softmax(outputs)
                vals = p[mask, labels] # min values are prioritized
                margins = vals.cpu().numpy()
        ## prob dist
        proba = margins
        proba = 1 / (proba + 1e-7) # min values get highest scores/ priorities
    ...
    ### ------------------------------------- ###
    ###  ///// Min Confidence Rehearsal \\\\\ ###
    ### ------------------------------------- ###
    def rehearsal_min_conf(self, opq, pq, latent_dict, rehearsal_ixs, num_iters): 
        start_time = time.time()
        total_loss = utils.CMA()
        params = self.get_layerwise_params(self.classifier_F, self.sleep_lr)
        classifier_F = self.classifier_F.cuda()
        ...
        lr_scheduler = optim.lr_scheduler.OneCycleLR(optimizer, 
            max_lr=self.sleep_lr, steps_per_epoch=num_iter, epochs=1)
        _, labels0, margins, _, ts = self.sample_min_conf(opq, pq, latent_dict, rehearsal_ixs, k)
        sampling_time = ts

        for i in range(num_iter):
            chosen_rehearsal_ixs, labels0, margins, indices, ts = self.sample_min_conf(opq, pq, latent_dict, 
                    rehearsal_ixs, k, labels0, margins, i+1)
            sampling_time += ts #(ts1+ts2)
            codes = np.empty((k, self.num_feats, self.num_feats, self.num_codebooks), dtype=np.uint8) # 50x14x14x8
            labels = torch.empty(k, dtype=torch.long).cuda() # 32
    ...
    ###  ///// Min Confidence Rehearsal \\\\\ ###
    ### ------------------------------------- ###
    def rehearsal_min_logit(self, opq, pq, latent_dict, rehearsal_ixs, num_iters): 
        start_time = time.time()
        total_loss = utils.CMA()
        params = self.get_layerwise_params(self.classifier_F, self.sleep_lr)
        classifier_F = self.classifier_F.cuda()
        ...
        lr_scheduler = optim.lr_scheduler.OneCycleLR(optimizer, 
            max_lr=self.sleep_lr, steps_per_epoch=num_iter, epochs=1)
        _, labels0, margins, _, ts = self.sample_min_logit(opq, pq, latent_dict, rehearsal_ixs, k)
        sampling_time = ts

        for i in range(num_iter):
            chosen_rehearsal_ixs, labels0, margins, indices, ts = self.sample_min_logit(opq, pq, latent_dict, 
                    rehearsal_ixs, k, labels0, margins, i+1)
            sampling_time += ts #(ts1+ts2)
            codes = np.empty((k, self.num_feats, self.num_feats, self.num_codebooks), dtype=np.uint8) # 50x14x14x8
            labels = torch.empty(k, dtype=torch.long).cuda() # 32
            ...
            lr_scheduler.step()
            total_loss.update(loss.item())
            ### update min logit distance ###
            conf = output[mask, labels]
            vals = abs(conf) # min values are prioritized
            margins[indices] = vals.detach().cpu().numpy()
            if (i+1) % 5000 == 0 or i == 0 or (i+1) == num_iter:
                print("Iter:", (i+1), "--Loss: %1.5f" % total_loss.avg)
    ...
    ###  ///// Min Rehearsal \\\\\ ###
    ### ------------------------- ###
    def rehearsal_minrep(self, opq, pq, latent_dict, rehearsal_ixs, 
        new_class_list, num_iters):
        start_time = time.time()
        total_loss = utils.CMA()
        params = self.get_layerwise_params(self.classifier_F, self.sleep_lr)
    ...
    ### -------- GRASP Rehearsal --------- ###
    ### ---------------------------------- ###
    def rehearsal_grasp(self, opq, pq, latent_dict, rehearsal_ixs, new_class_list, num_iters):
        start_time = time.time()
        total_loss = utils.CMA()
        model_old = copy.deepcopy(self.classifier_F)
        model_old.eval()
        model_old.cuda()
        params = self.get_layerwise_params(self.classifier_F, self.sleep_lr)
        classifier_F = self.classifier_F.cuda()
        ...
        codes = np.empty((len(to_be_replayed), self.num_feats, 
                self.num_feats, self.num_codebooks), dtype=np.uint8)
        labels = torch.empty(len(to_be_replayed), dtype=torch.long)
        for ii, v in enumerate(to_be_replayed):
            v = v.item()
            codes[ii] = latent_dict[v][0]
            labels[ii] = torch.from_numpy(latent_dict[v][1])
        ...
        for i in range(num_iter):
            ...
            ##### Separate old and new samples ######
            new_class_list = np.array(new_class_list, dtype=np.int32)
            label_list = np.array(labels_batch, dtype=np.int32)
            idxs_old = [0 if k in new_class_list else 1 for k in label_list] # 0 means new class & 1 means old class
            idxs_old = np.array(idxs_old)
            num_old = np.count_nonzero(idxs_old) # num of old samples
            assert np.count_nonzero(idxs_old==0) + np.count_nonzero(idxs_old==1)  == len(labels_batch)
            idxs_old0 = idxs_old == 1
            
            if num_old < len(labels_batch):
                data_new = data_batch_recon[~idxs_old0]
                data_old = data_batch_recon[idxs_old0]
                labels_new = labels_batch[~idxs_old0]
                labels_old = labels_batch[idxs_old0]
                output_new = classifier_F(data_new)
                output_old = classifier_F(data_old)
                output_kd = model_old(data_old) ## old classes & old model
                loss1 = criterion(output_new, labels_new.cuda()) ## new classes
                loss2 = criterion(output_old, labels_old.cuda()) ## old classes
                loss3 = criterion2(output_old, output_kd)
                loss = loss1 + 0.9 * loss2 + 0.1 * loss3
            else:
                ### fit on replay mini-batch
                output = classifier_F(data_batch_recon)  # 64 x 80 x 14 x 14
                loss = criterion(output, labels_batch.cuda())
    ...
    ### --------------------------------------------- ###
    def rehearsal_uniform_bal(self, opq, pq, latent_dict, rehearsal_ixs, new_class_list, num_iters):
        start_time = time.time()
        total_loss = utils.CMA()
        model_old = copy.deepcopy(self.classifier_F)
        model_old.eval()
    ...
    def rehearsal_uniform(self, opq, pq, latent_dict, rehearsal_ixs, num_iters):
        ...
        for s in range(num_iter):   
            chosen_rehearsal_ixs = np.random.choice(rehearsal_ixs, size=bs, replace=False) # 512
