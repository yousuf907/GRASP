...
import os
import numpy as np
from MNModel_derpp import MNModel
#import wandb
from imagenet_base_init import *
torch.multiprocessing.set_sharing_strategy('file_system')

### MobileNet / ImageNet / DERpp / Veridical Rehearsal ###
...
def get_base_init_loader(images_dir, label_dir, split, min_class, max_class, batch_size=128, return_item_ix=False):
    data_loader = utils_imagenet2.get_imagenet_base_init_loader(images_dir + '/' + split, label_dir, split,
        batch_size=batch_size, shuffle=False, min_class=min_class, max_class=max_class, return_item_ix=return_item_ix)
    return data_loader
...
def streaming(args, mnet):
    ...
    latent_dict, rehearsal_ixs, class_id_to_item_ix_dict, class_id_dist, recent_class_list = mnet.update_buffer(train_loader_curr,
        latent_dict, rehearsal_ixs, class_id_to_item_ix_dict, class_id_dist, counter)  # 0-100
    ...
    for class_ix in range(args.streaming_min_class, args.streaming_max_class, args.class_increment):
        ...
        latent_dict, rehearsal_ixs, class_id_to_item_ix_dict, class_id_dist, recent_class_list = mnet.update_buffer(train_loader_curr,
            latent_dict, rehearsal_ixs, class_id_to_item_ix_dict, class_id_dist, counter)  # 0-50, 50-100, 150-200
        print('Counter AFTER buffer update.', counter.count)
        ...
        if rotation < sleep_freq: # 1 < 2
            print('\nSTATUS -> Skipping Online/ Awake Phase...')
        ...
        #/////////////////////////////////////////////// SLEEP PHASE /////////////////////////////////////////////
            ...
            sel_idxs = mnet.grasp_sampling(class_id_to_item_ix_dict, class_id_dist, seen_class_list, num_iter)
            ...
            ### Train DNN
            mnet.derpp_rehearsal(replay_loader, new_class_list, num_iter)
