...
class StreamingLDA(nn.Module):
    ...
    def __init__(self, input_shape, num_classes, shrinkage_param=1e-2, backbone=None, streaming_update_sigma=True,
        ...
        self.Sigma = torch.ones((input_shape, input_shape), dtype=torch.float64).to(self.device)  # covariance
        self.num_updates = 0
        self.Lambda = torch.zeros_like(self.Sigma).to(self.device)
        self.prev_num_updates = -1

    def reset_slda(self):
        print("Resetting SLDA parameters since representations have changed..")
        self.muK = torch.zeros((self.num_classes, self.input_shape), dtype=torch.float64).to(self.device)
        self.cK = torch.zeros(self.num_classes).to(self.device)
        self.Sigma = torch.ones((self.input_shape, self.input_shape), dtype=torch.float64).to(self.device)  # covariance
        self.num_updates = 0
        self.Lambda = torch.zeros_like(self.Sigma).to(self.device)
        self.prev_num_updates = -1
    ...
    def predict(self, X, return_probas=False):
        ...
        if self.prev_num_updates != self.num_updates:
            ...
            Lambda = torch.pinverse((1 - self.shrinkage_param) * self.Sigma +
