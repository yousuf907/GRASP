...
import torch
import os
import utils_imagenet_pq as utils_imagenet2
import utils_pq as utils
from retrieve_any_layer import ModelWrapper
from utils_pq import build_classifier
import functools
...
def extract_base_init_features(imagenet_path, label_dir, extract_features_from, classifier_ckpt, arch,
    ...
    base_train_loader = utils_imagenet2.get_imagenet_data_loader(imagenet_path + '/train', label_dir, split='train',
...
def fit_pq(feats_base_init, labels_base_init, item_ix_base_init, num_channels, spatial_feat_dim, 
    ...
    #d_out=240
    #train_data_base_init, mat = whiten(train_data_base_init, num_channels, d_out)

    print('\nTraining Product Quantizer')
    start = time.time()
    nbits = int(np.log2(codebook_size))
    pq = faiss.ProductQuantizer(num_channels, num_codebooks, nbits)
    #pq = faiss.ProductQuantizer(d_out, num_codebooks, nbits)
    ...
    for i in range(0, num_samples, mb):
        ...
        #data_batch = mat.apply_py(data_batch) # 49 mb x pca_dim
