...
def get_imagenet_replay_loader(dirname, label_dir, split, idxs, batch_size=128, shuffle=False,
...
def get_imagenet_base_init_loader(dirname, label_dir, split, batch_size=128, shuffle=False, min_class=0, max_class=None,
            sampler=None, batch_sampler=None, dataset_name='imagenet', return_item_ix=False, num_workers=8):
    # filter out only the indices for the desired class
    if max_class is not None:
        _labels = np.load(
