...
class Memory(object):
    ...
    def append(self, example, label, dist, task):
        self.examples.append(example)
        self.labels.append(label)
        self.tasks.append(task)
        self.dists.append(dist)
        self.d_list.append(dist)

    def reset(self, task):
        self.dists = [self.d_list[i] for i in range(len(self.d_list))]
    ...
    def get_random_batch(self, batch_size, task_id=None):
        if task_id is None:
            ...
            ixs_curr_task = np.array(index)
            dist_curr_task = [self.dists[i] for i in index]
            dist_curr_task2 = np.array(dist_curr_task)
            max_dist = np.max(dist_curr_task2)
            probas = 1 / (dist_curr_task2 + 1e-10) # min distances get highest scores/ priorities > easy examples
            p_curr_task = probas / np.linalg.norm(probas, ord=1)  # sum to 1

            if np.count_nonzero(p_curr_task) < len(p_curr_task):
                self.reset(task_id)
                dist_curr_task = [self.dists[i] for i in index]
                dist_curr_task2 = np.array(dist_curr_task)
                max_dist = np.max(dist_curr_task2)
            ...
            sel_idx = np.random.choice(ixs_curr_task, size=batch_size, replace=False, p=p_curr_task)

            mini_examples = [self.examples[i] for i in sel_idx]
            mini_labels = [self.labels[i] for i in sel_idx]
            mini_tasks = [self.tasks[i] for i in sel_idx]

            ###
            for i in sel_idx:
                self.dists[i] += max_dist
            ###
...
def random_select_samples_to_store(buffer, model, data_loader, task_id):
    model.eval()
    x_list = []
    y_list = []
    p_list = [] # penultimate features
    ...
    with torch.no_grad():
        for x, mask, y in data_loader:
            ...
            x_list.append(x)
            y_list.append(y)
            p = model.penultimate(x) # batch x 768
            p_list.append(p)
    x_list = torch.cat(x_list, dim=0).data.cpu().numpy()
    y_list = torch.cat(y_list, dim=0).data.cpu().numpy()
    p_list = torch.cat(p_list, dim=0).data.cpu().numpy() # N x 768

    task_mean = np.mean(p_list, axis=0) # mean of current task
    task_mean = np.reshape(task_mean, (-1, 768))
    dist = cosine_distances(task_mean, p_list)
    dist = dist.squeeze() # N

    permutations = np.random.permutation(len(x_list))
    index = permutations[:int(args.store_ratio * len(x_list))]
    for j in index:
        buffer.append(x_list[j], y_list[j], dist[j], task_id)
...
def main():
    ...
    for task_id in range(task_num):
        ...
        for epoch in range(args.epochs[task_id]):
            ...
            ### Reset
            buffer.reset(task_id) # reset if using multiple epochs
        ...
        random_select_samples_to_store(buffer, model, data_loader, task_id)
