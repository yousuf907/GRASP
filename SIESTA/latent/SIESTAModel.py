...
class SIESTAModel(object):
    ...
    def grasp_sampling(self, dist_dict, class_list, num_iters):
        ...
        while count < budget:
            for i in range(len(class_list)):
                ...
                ## sample
                sel_idx = np.random.choice(ixs_current_class, size=1, replace=False, p=p_curr_class) # replace does not matter for 1 sample
                count += 1
                ###
        ...
        rehearsal_idxs = np.array(rehearsal_idxs, dtype=np.int32) ## contains all IDs of samples that will be rehearsed
    ...
    def uniform_balanced(self, latent_dict, rehearsal_ixs, batch_size, num_iters):
        ...
        while count < budget:
            for c in class_list:
                ...
                sel_idx = np.random.choice(ixs_current_class, size=k, replace=False) # replace does not matter for a single sample
                count += k
    ...
    ### -------- GRASP Rehearsal --------- ###
    ### ---------------------------------- ###
    def rehearsal_grasp(self, opq, pq, latent_dict, rehearsal_ixs, iterations):
        start_time = time.time()
        total_loss = utils.CMA()
        params = self.get_layerwise_params(self.classifier_F, self.sleep_lr)
        classifier_F = self.classifier_F.cuda()
    ...
    ### -------- GRASP Rehearsal --------- ###
    ### ---------------------------------- ###
    def rehearsal_grasp_easy(self, opq, pq, latent_dict, rehearsal_ixs, iterations):
        start_time = time.time()
        total_loss = utils.CMA()
        params = self.get_layerwise_params(self.classifier_F, self.sleep_lr)
        classifier_F = self.classifier_F.cuda()
        ...
        ### GRASP Rehearsal (balanced) ###
        dist_dict, class_list = self.dist_to_centroid(opq, pq, latent_dict, rehearsal_ixs)
        to_be_replayed = self.grasp_easy_sampling(dist_dict, class_list, num_iter)
        sampling_time = time.time() - sampling_start_time
        total_sampling_time = int(sampling_time / 60)  # in minutes
        print("\nTotal Sampling Time (in mins):", total_sampling_time)
        total_num_samples = len(to_be_replayed)
    ...
    ### -------- GRASP Rehearsal --------- ###
    ### ---------------------------------- ###
    def rehearsal_grasp_hard(self, opq, pq, latent_dict, rehearsal_ixs, iterations):
        start_time = time.time()
        total_loss = utils.CMA()
        params = self.get_layerwise_params(self.classifier_F, self.sleep_lr)
        classifier_F = self.classifier_F.cuda()
        ...
        ### GRASP Rehearsal (balanced) ###
        dist_dict, class_list = self.dist_to_centroid(opq, pq, latent_dict, rehearsal_ixs)
        to_be_replayed = self.grasp_hard_sampling(dist_dict, class_list, num_iter)
        sampling_time = time.time() - sampling_start_time
        total_sampling_time = int(sampling_time / 60)  # in minutes
        print("\nTotal Sampling Time (in mins):", total_sampling_time)
        total_num_samples = len(to_be_replayed)
    ...
    ### --------------------------------------------- ###
    def rehearsal_uniform_bal(self, opq, pq, latent_dict, rehearsal_ixs, iterations):
        start_time = time.time()
        total_loss = utils.CMA()
        params = self.get_layerwise_params(self.classifier_F, self.sleep_lr)
        classifier_F = self.classifier_F.cuda()
