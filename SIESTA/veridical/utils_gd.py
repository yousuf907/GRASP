...
from mobilenet_modified_gelu_ER import * # Cosine with bias
...
def safe_load_dict(model, new_model_state, should_resume_all_params=False):
    ...
    #for k in new_model_state:
    for k in old_model_state:
    #    if 'sigma' not in k:
    #        assert torch.equal(old_model_state[k].cpu(), new_model_state['module.' + k[len("model."):]].cpu()), k
        assert torch.equal(old_model_state[k].cpu(), new_model_state[k].cpu()), k
    print("Successfully performed sanity check after loading pre-trained weights!!")
