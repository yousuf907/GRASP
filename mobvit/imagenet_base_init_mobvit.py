...
import torch
import os
import utils_imagenet_mobvit as utils_imagenet2
import utils_mobvit as utils
from retrieve_any_layer import ModelWrapper
from utils_mobvit import build_classifier
import functools
...
def extract_base_init_features(imagenet_path, label_dir, extract_features_from, classifier_ckpt, arch,
                               max_class, num_channels, spatial_feat_dim, batch_size=64):
...
def fit_opq(feats_base_init, labels_base_init, item_ix_base_init, num_channels, spatial_feat_dim, 
    num_codebooks, codebook_size, batch_size=64, counter=utils.Counter()):
    start = time.time()
    ...
    pq = faiss.ProductQuantizer(num_channels, num_codebooks, nbits) # 128, 8, 8
