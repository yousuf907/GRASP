...
import os
import numpy as np
from MNModel_vit import MNModel
from imagenet_base_init_mobvit import *
torch.multiprocessing.set_sharing_strategy('file_system')

### SIESTA / MobileViT / ImageNet / Latent Rehearsal ###

def get_data_loader(images_dir, label_dir, split, min_class, max_class, batch_size=128, return_item_ix=False):
    data_loader = utils_imagenet2.get_imagenet_data_loader(images_dir + '/' + split, label_dir, split, batch_size=batch_size,
...
def streaming(args, mnet):
    ...
    if args.resume_full_path is not None:
        ...
        print('\nMobViT accuracy on BASE-INIT classes: top1=%0.2f%% -- top5=%0.2f%%' % (top1_base, top5_base))
        ...
        ################
        ## INITIALIZING SIESTA (fc IN CLASSIFIER_F) WITH BASE INIT CLASSES
        ...
        #mnet.joint_train_base_no_aug(opq, pq, latent_dict, rehearsal_ixs, init_test_loader, args.save_dir, args.ckpt_file)
    ...
    Wt=0 # tracking wake test time
    St0=0 # tracking pre-sleep test time
    St=0 # tracking post-sleep test time
    ...
    for class_ix in range(args.streaming_min_class, args.streaming_max_class, args.class_increment):
        ...
        if rotation < sleep_freq: # 1 < 2
            ...
            ### All Seen Classes ###
            probas_dnn, true_dnn = mnet.predict(test_loader_all, opq, pq)
        ...
        #/////////////////////////////////////////////// SLEEP PHASE /////////////////////////////////////////////
            ...
            ## Wake update before sleep
            mnet.update_new_nodes(recent_class_list)

            print('\nSTATUS -> SLEEP... Number of Sleeps =', num_sleep)
            print('Rehearsal Cycle Begins..')
            ### Train DNN
            #print("Uniform Balanced Rehearsal")
            #mnet.rehearsal_uniform_bal(opq, pq, latent_dict, rehearsal_ixs, num_iter)
            #print("Uniform Rehearsal")
            #mnet.rehearsal_uniform(opq, pq, latent_dict, rehearsal_ixs, num_iter)

            print("GRASP Rehearsal")
            mnet.rehearsal_grasp(opq, pq, latent_dict, rehearsal_ixs, num_iter)

            #print("Min Margin Rehearsal")
            #mnet.rehearsal_margin(opq, pq, latent_dict, rehearsal_ixs, num_iter)

            #print("\Max Loss Rehearsal")
            #mnet.rehearsal_ml(opq, pq, latent_dict, rehearsal_ixs, num_iter)

            #print("Min Replay")
            #mnet.rehearsal_minrep(opq, pq, latent_dict, rehearsal_ixs, new_class_list, num_iter)
    ...
    print("Total runtime without test time:", tt)
    ## /// END /// ##
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_channels', type=int, default=128)  # number of channels where features are extracted
    parser.add_argument('--spatial_feat_dim', type=int, default=16)  # spatial dimension of features being extracted

    parser.add_argument('--weight_decay', type=float, default=1e-2)  # weight decay for network
    ...
    parser.add_argument('--sleep_lr', type=float, default=1e-4)  # starting lr for sleep offline training
