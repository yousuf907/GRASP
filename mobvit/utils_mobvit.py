...
#from mobilevit_models import * # MobViT with BN
from mobvit_models_gn_ws import * # MobViT with GN and WS
...
class RoIAlignCrop(object):
    ...
    def get_params(self, img, scale, ratio):
        ...
        for attempt in range(10):
            ...
            log_ratio = (math.log(ratio[0]), math.log(ratio[1]))
            aspect_ratio = math.exp(random.uniform(*log_ratio))
            w = int(round(math.sqrt(target_area * aspect_ratio))) # width has to be integer
            h = int(round(math.sqrt(target_area / aspect_ratio))) # height has to be integer
            if w <= img.size(1) and h <= img.size(2):
                i = random.uniform(0, img.size(2) - h) # needs to be float
                j = random.uniform(0, img.size(1) - w) # needs to be float
                return i, j, h, w
        # Fallback to central crop
        in_ratio = img.size(1) / img.size(2)
        if (in_ratio < min(ratio)):
    ...
    def __call__(self, img):
        i, j, h, w = self.get_params(img, self.scale, self.ratio)
        box = torch.Tensor([0, i, j, i+h, j+w]).cuda()  # [id, x1, y1, x2, y2]
        crop = roi_align(img.unsqueeze(0), box.unsqueeze(0), [self.size, self.size], aligned=True)
